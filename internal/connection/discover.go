...
// for each bit bucket populate the closest nodes in the network
func (node *Node) Handel_discover() {

	// inserting own id into buckets
	...
	if config.MetaData.WellKnownPort != config.MetaData.Port {
		...
		for i := 0; i < 256; i++ {
			id := i / 8
			msk := i % 8
			// calculate the node id mask for each 256 bit compared to its node id
